...
(() => {
  ...
  const hemi = new THREE.HemisphereLight(0xb8c0cc, 0x2a2e36, 0.95);
  scene.add(hemi);
  const dir = new THREE.DirectionalLight(0xe8d5a3, 0.55);
  ...
  function makeGridTex() {
    ...
    g.fillStyle = "#3d4452"; g.fillRect(0, 0, 128, 128);
    g.fillStyle = "#454c5c"; g.fillRect(0, 0, 64, 64); g.fillRect(64, 64, 64, 64);
    g.strokeStyle = "rgba(20,22,28,0.55)"; g.lineWidth = 2; g.strokeRect(1, 1, 126, 126);
  ...
  for (let i = 0; i < 18; i++) {
    ...
    const m = new THREE.Mesh(new THREE.BoxGeometry(2 + Math.random() * 4, h, 2 + Math.random() * 3), blockMat);
    const a = Math.random() * Math.PI * 2, r = 28 + Math.random() * 40;
    m.position.set(Math.cos(a) * r, h / 2, Math.sin(a) * r); scene.add(m);
  }
  ...
  function parsePrompt(text) {
    ...
    const colorMap = [["red",0x8a3030],["blue",0x2c4a7a],["yellow",0xc4a22a],["gold",0xc4a22a],["green",0x3a6a3e],["white",0xd8d2c8],["cream",0xd8d2c8],["black",0x1c1c20],["dark",0x2a2a30],["purple",0x5a3a6a],["orange",0xb86428],["teal",0x2a6a68],["gray",0x6a7080],["grey",0x6a7080],["burgundy",0x6a2838]];
    let color = null;
    for (const [k, v] of colorMap) if (t.includes(k)) { color = v; break; }
    ...
    if (/(car|sedan|vehicle|truck|van|bus)/.test(t)) return { kind: "car", color: color || 0x6a2838, raw: t };
    if (/(woman|girl)/.test(t)) return { kind: "woman", color, raw: t };
    if (/(raincoat|stranger)/.test(t)) return { kind: "raincoat", color, raw: t };
    if (/(suit|man|person|people|figure|someone|human)/.test(t)) return { kind: "suit", color, raw: t };
    if (/(lamp|light|lantern)/.test(t)) return { kind: "lamp", color, raw: t };
    ...
    if (/(door|portal|gate)/.test(t)) return { kind: "door", color: color || 0x3a2a22, raw: t };
    if (/(house|building|wall|tower)/.test(t)) return { kind: "building", color: color || 0x5a6270, raw: t };
    if (/(cube|box|stone)/.test(t)) return { kind: "cube", color: color || 0x6a7080, raw: t };
  ...
  function spawnFromSpec(spec, pos, rotY) {
    ...
    const addShadow = (w = 1.2) => {
      const sh = new THREE.Mesh(new THREE.CircleGeometry(w, 10), new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.28 }));
    ...
    if (spec.kind === "raincoat") { g.add(dummyPerson(0x6a7380)); addShadow(0.55); }
      ...
      for (let i = 0; i < 8; i++) {
        const st = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.22, 0.45), lamb(0x6a7080));
        st.position.set(0, 0.11 + i * 0.22, -i * 0.4); g.add(st);
      }
      addShadow(1.1);
      ...
      const leg1 = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.45, 0.5), lamb(0x2a2a28));
      const leg2 = leg1.clone(); leg1.position.set(-0.65, 0.22, 0); leg2.position.set(0.65, 0.22, 0);
      ...
      const inner = new THREE.Mesh(new THREE.BoxGeometry(1.0, 2.0, 0.08), new THREE.MeshLambertMaterial({ color: 0x10141c, emissive: 0x1a2436, emissiveIntensity: 0.6 }));
      inner.position.set(0, 1.15, 0.04); g.add(frame, inner); addShadow(0.7);
    } else if (spec.kind === "building") {
      const h = 3 + Math.random() * 4;
      const b = new THREE.Mesh(new THREE.BoxGeometry(3.2, h, 2.4), lamb(spec.color)); b.position.y = h / 2; g.add(b); addShadow(1.8);
      ...
      const core = new THREE.Mesh(new THREE.IcosahedronGeometry(0.55, 0), new THREE.MeshLambertMaterial({ color: spec.color, emissive: spec.color, emissiveIntensity: 0.18 }));
  ...
  function applySky(spec) {
    const t = spec.raw || "";
    let fog = 0x8a93a0, hemiSky = 0xb8c0cc, hemiG = 0x2a2e36, dirC = 0xe8d5a3, intensity = 0.55;
    if (/red|dusk|sunset|blood/.test(t)) { fog = 0xa07060; hemiSky = 0xc88870; hemiG = 0x2a1814; dirC = 0xffb080; intensity = 0.7; }
    ...
    else if (/blue|cold|winter/.test(t)) { fog = 0x6a88a8; hemiSky = 0xa8c0d8; hemiG = 0x1a2430; dirC = 0xd0e4ff; intensity = 0.5; }
    else if (/green|forest/.test(t)) { fog = 0x6a8070; hemiSky = 0xb0c4a8; hemiG = 0x1c2418; dirC = 0xd8e8b0; intensity = 0.5; }
    else if (/clear|white|bright/.test(t)) { fog = 0xc8c8c4; hemiSky = 0xe8e8e4; hemiG = 0x50545c; dirC = 0xffffff; intensity = 0.9; }
    state.fogColor.setHex(fog);
    scene.background = state.fogColor.clone(); scene.fog.color.copy(state.fogColor);
    renderer.setClearColor(fog, 1);
    hemi.color.setHex(hemiSky); hemi.groundColor.setHex(hemiG);
    dir.color.setHex(dirC); dir.intensity = intensity;
  }
  function frontSpawnPos(dist) {
    const dirv = new THREE.Vector3(); camera.getWorldDirection(dirv); dirv.y = 0; dirv.normalize();
    return new THREE.Vector3(camera.position.x + dirv.x * dist, 0, camera.position.z + dirv.z * dist);
  }
  function weave(text) {
    if (state.generating) return;
    const raw = (text || document.getElementById("prompt").value || "").trim();
    if (!raw) return;
    const spec = parsePrompt(raw); spec.prompt = raw; state.generating = true;
    const overlay = document.getElementById("weave");
    document.getElementById("weave-line").textContent = raw.toUpperCase();
    overlay.classList.add("show");
    setTimeout(() => {
      if (spec.kind === "sky") applySky(spec);
      else {
        const obj = spawnFromSpec(spec, frontSpawnPos(6.2), state.yaw);
        obj.scale.set(0.01, 0.01, 0.01);
        const start = performance.now();
        const grow = () => {
          const k = Math.min(1, (performance.now() - start) / 420);
          obj.scale.setScalar(1 - Math.pow(1 - k, 3));
          if (k < 1) requestAnimationFrame(grow);
        };
        grow();
      }
      overlay.classList.remove("show"); state.generating = false;
      setStatus("WOVE · " + raw.toUpperCase().slice(0, 28));
    }, 800);
  }
  function clearVoid(keepSky) {
    while (worldRoot.children.length) worldRoot.remove(worldRoot.children[0]);
    state.objects = [];
    if (!keepSky) applySky({ raw: "gray fog void" });
  }
  function serialize() {
    return { name: state.currentName, savedAt: Date.now(), fog: state.fogColor.getHex(), yaw: state.yaw, pitch: state.pitch,
      pos: { x: camera.position.x, y: camera.position.y, z: camera.position.z },
      objects: state.objects.map((o) => ({ spec: o.userData.spec, x: o.position.x, z: o.position.z, rot: o.rotation.y })) };
  }
  function hexToSkyHint(hex) {
    if (!hex) return "gray fog";
    if (hex < 0x303840) return "night dark";
    return "gray fog";
  }
  function loadDream(data) {
    clearVoid(true);
    state.currentName = data.name || "untitled dream";
    applySky({ raw: hexToSkyHint(data.fog) });
    state.fogColor.setHex(data.fog || 0x8a93a0);
    scene.background = state.fogColor.clone(); scene.fog.color.copy(state.fogColor);
    renderer.setClearColor(state.fogColor.getHex(), 1);
    if (data.pos) camera.position.set(data.pos.x, data.pos.y, data.pos.z);
    state.yaw = data.yaw || 0; state.pitch = data.pitch || -0.08;
    (data.objects || []).forEach((o) => spawnFromSpec(o.spec, new THREE.Vector3(o.x, 0, o.z), o.rot));
    setStatus("RETURNED · " + state.currentName.toUpperCase());
  }
  ...
  function saveDream() {
    const name = (document.getElementById("dream-name").value || state.currentName || "untitled").trim();
    state.currentName = name;
    const list = allDreams().filter((d) => d.name !== name);
    list.unshift(serialize()); persist(list.slice(0, 24)); renderDreamList();
    setStatus("SAVED · " + name.toUpperCase());
  }
  function renderDreamList() {
    const el = document.getElementById("dream-list"); const list = allDreams();
    if (!list.length) { el.innerHTML = '<p class="sub">No saved dreams yet.</p>'; return; }
    el.innerHTML = list.map((d, i) => `<div class="dream-item"><div><b>${escapeHtml(d.name)}</b><br><span>${new Date(d.savedAt).toLocaleString()} · ${d.objects.length} forms</span></div><div class="row" style="margin:0"><button class="btn" data-load="${i}">ENTER</button><button class="btn ghost" data-del="${i}">×</button></div></div>`).join("");
    el.querySelectorAll("[data-load]").forEach((b) => { b.onclick = () => { loadDream(allDreams()[+b.dataset.load]); toggleDesign(false); }; });
    el.querySelectorAll("[data-del]").forEach((b) => { b.onclick = () => { const list2 = allDreams(); list2.splice(+b.dataset.del, 1); persist(list2); renderDreamList(); }; });
  }
  function escapeHtml(s) { return String(s).replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c])); }
  function setStatus(t) { document.getElementById("status").textContent = t; }
  function toggleDesign(force) {
    state.designOpen = force == null ? !state.designOpen : force;
    document.getElementById("design-panel").classList.toggle("open", state.designOpen);
  }
  function startGame() {
    document.getElementById("title-screen").style.display = "none";
    state.started = true; state.paused = false;
    document.getElementById("pause").classList.remove("open");
    setStatus("THE VOID IS LISTENING");
  }
  function wake() {
    state.started = false; state.paused = false;
    document.getElementById("title-screen").style.display = "";
    document.getElementById("pause").classList.remove("open"); toggleDesign(false);
  }
  function togglePause() {
    if (!state.started) { startGame(); return; }
    if (state.designOpen) { toggleDesign(false); return; }
    state.paused = !state.paused;
    document.getElementById("pause").classList.toggle("open", state.paused);
  }
  window.addEventListener("keydown", (e) => {
    state.keys[e.code] = true;
    if (e.code === "Tab" || e.code === "KeyE") { e.preventDefault(); toggleDesign(); }
    if (e.code === "Escape") togglePause();
    if (e.code === "Enter" && state.designOpen) weave();
    if (e.code === "Space" && !state.started) startGame();
  });
  window.addEventListener("keyup", (e) => { state.keys[e.code] = false; });
  function bindStick(el, knob, target) {
    let pid = null;
    const go = (x, y) => {
      const r = el.getBoundingClientRect();
      let dx = x - (r.left + r.width / 2), dy = y - (r.top + r.height / 2);
      const max = r.width * 0.38, mag = Math.hypot(dx, dy) || 1;
      if (mag > max) { dx *= max / mag; dy *= max / mag; }
      knob.style.transform = "translate(calc(-50% + " + dx + "px), calc(-50% + " + dy + "px))";
      target.x = dx / max; target.y = dy / max;
    };
    const end = () => { pid = null; knob.style.transform = "translate(-50%,-50%)"; target.x = 0; target.y = 0; };
    el.addEventListener("pointerdown", (e) => { pid = e.pointerId; el.setPointerCapture(pid); go(e.clientX, e.clientY); });
    el.addEventListener("pointermove", (e) => { if (e.pointerId === pid) go(e.clientX, e.clientY); });
  ...
  let padIndex = null;
  window.addEventListener("gamepadconnected", (e) => { padIndex = e.gamepad.index; setStatus("BACKBONE CONNECTED"); });
  window.addEventListener("gamepaddisconnected", () => { padIndex = null; setStatus("WALK THE VOID"); });
  const padPrev = {};
  function readPad() {
    const pads = navigator.getGamepads ? navigator.getGamepads() : [];
    const gp = (padIndex != null && pads[padIndex]) || Array.from(pads).find(Boolean);
    if (!gp) return;
    const dead = (v) => (Math.abs(v) < 0.18 ? 0 : v);
    if (!state.designOpen) {
      state.move.x = dead(gp.axes[0] || 0); state.move.y = dead(gp.axes[1] || 0);
      state.look.x = dead(gp.axes[2] || 0); state.look.y = dead(gp.axes[3] || 0);
    }
    const pressed = (i) => gp.buttons[i] && gp.buttons[i].pressed;
    const edge = (i) => { const p = pressed(i), was = padPrev[i]; padPrev[i] = p; return p && !was; };
    if (edge(9) || edge(8)) togglePause();
    if (edge(5) || edge(4) || edge(3)) toggleDesign();
    if (edge(0) && !state.started) startGame();
  ...
  document.getElementById("chips").onclick = (e) => {
    ...
    document.getElementById("prompt").value = b.dataset.fill;
  };
  renderDreamList();
  function resize() {
    const w = window.innerWidth, h = window.innerHeight;
    const internalW = 640, internalH = Math.max(280, Math.round(640 * (h / w)));
    renderer.setSize(internalW, internalH, false);
    canvas.style.width = w + "px"; canvas.style.height = h + "px";
    camera.aspect = internalW / internalH; camera.updateProjectionMatrix();
  }
  ...
  function tick() {
    ...
    state.yaw -= state.look.x * 1.8 * dt;
    state.pitch -= state.look.y * 1.4 * dt;
    state.pitch = Math.max(-1.2, Math.min(0.6, state.pitch));
